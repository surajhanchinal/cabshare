...
namespace cabshare
{
    public class JoinCard
    {
        ...
        public static async Task<int> Cards(Activity activity,ConnectorClient connector,List<Request> requests)
        {
            ...
            {
                string card = String.Format("DATE : {0}    TIME : {1}    FROM : {2}    TO : {3}    VACANCY : {4}    MEMBERS : {5}", b.date1.Value.ToShortDateString(), b.time1.ToString(), b.origin.TrimEnd(), b.destination.TrimEnd(),b.MAXNO,b.names);
